...
static void validateSlices(TextureType type, TextureData* slices[6], int sliceCount) {
  if (type == TEXTURE_CUBE) {
    if (sliceCount != 6) {
      lovrThrow("Cube textures must have 6 images");
    }
    ...
    if (width != height) {
      lovrThrow("Cube textures must be square");
    }

    for (int i = 1; i < sliceCount; i++) {
      if (slices[i]->width != width || slices[i]->height != height) {
        lovrThrow("All textures in a cube texture must have the same dimensions");
      }
    }
  } else if (type == TEXTURE_2D && sliceCount != 1) {
    lovrThrow("2D textures can only have a single image");
  }
}
...
Texture* lovrTextureCreate(TextureType type, TextureData* slices[6], int sliceCount) {
  ...
  TextureWrap wrap = { .s = wrapMode, .t = wrapMode, .r = wrapMode };
...
void lovrTextureSetWrap(Texture* texture, TextureWrap wrap) {
  ...
  if (texture->type == TEXTURE_CUBE) {
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_R, wrap.r);
  }
}
